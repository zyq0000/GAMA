...
class T5ForImageSearch(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        if labels is not None:
            loss_ce = self.loss_ce(logits.view(-1, logits.size(-1)), labels.view(-1))
            loss = loss_ce
    ...
    def prepare_inputs_for_generation(
        ...
        if "narrative_input_ids" in kwargs:
            output["narrative_input_ids"] = kwargs["narrative_input_ids"]
        if "narrative_attention_mask" in kwargs:
            output["narrative_attention_mask"] = kwargs["narrative_attention_mask"]
